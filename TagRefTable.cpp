 ...
 */
	...
	for(int i = 0; i < n; i++){
		...
		uint32_t refIdSub = *((uint32_t*)(data + entryOffset + 0x8));
		uint32_t refIdCenter = *((uint32_t*)(data + entryOffset + 0xc));
		uint32_t refId = *((uint32_t*)(data + entryOffset + 0x10));
		uint32_t parent = *((uint32_t*)(data + entryOffset + 0x14));

		entries.emplace_back(TagRefTableEntry((char*)&type,stringOffset,refIdSub,refIdCenter,refId,parent,std::string(stringData + stringOffset)));
	}
